...
io.on('connection', (socket) => {
 ...
 socket.on('subscribe', async (data: { pairs?: string[], channel?: string }) => {
  const pairs = data.pairs || (data.channel ? [data.channel.split(':')[1]] : []);
  
  if (!pairs || pairs.length === 0) {
    console.log('No pairs to subscribe to');
    return;
  }
