...
// ✅ FIXED VERSION - Uses reserve-based pricing for consistent token prices
...
export class PriceCalculatorEnhanced {
  ...
  static async calculateSwapPrice(
    ...
    // ✅ CRITICAL FIX: For WOPN pairs, calculate from swap rate (works correctly)
    // For non-WOPN pairs, use reserve-based pricing (was broken, now fixed)
    
    if (isToken0WOPN || isToken1WOPN) {
      // WOPN pairs - calculate prices and return result
      ...
      // ✅ FIXED: Non-WOPN pair - use database lookup with reserve-based pricing
      if (!db) {
  ...
  /**
   * ✅ FIXED: Calculate price for non-WOPN pairs using RESERVE RATIOS
   * This ensures prices stay consistent regardless of trade direction
   */
  private static async calculateCrossPairPrice(
    ...
    try {
      // ✅ CRITICAL FIX: Calculate prices from RESERVE RATIO, not swap amounts!
      // Reserve ratio = actual pool price, stays consistent
      const reservePriceAinB = reserveB / reserveA;  // How many B per A (from reserves)
      const reservePriceBinA = reserveA / reserveB;  // How many A per B (from reserves)
      ...
      // ✅ FIXED: Use reserve ratios to calculate missing prices
      if (tokenAPriceUSD === 0 && tokenBPriceUSD > 0) {
        // Calculate tokenA price from tokenB price using RESERVE ratio
        tokenAPriceUSD = reservePriceAinB * tokenBPriceUSD;
      } else if (tokenBPriceUSD === 0 && tokenAPriceUSD > 0) {
        // Calculate tokenB price from tokenA price using RESERVE ratio
        tokenBPriceUSD = reservePriceBinA * tokenAPriceUSD;
        ...
        // ✅ FIXED: If only one price is found, calculate the other using RESERVE ratio
        if (tokenAPriceUSD === 0 && tokenBPriceUSD > 0) {
          tokenAPriceUSD = reservePriceAinB * tokenBPriceUSD;
        } else if (tokenBPriceUSD === 0 && tokenAPriceUSD > 0) {
          tokenBPriceUSD = reservePriceBinA * tokenAPriceUSD;
      ...
      // Calculate volume based on actual swap
      ...
      return {
        ...
        token0Price: {
          ...
          priceInverse: reservePriceBinA  // ✅ Use reserve ratio for inverse
        },
        token1Price: {
          priceInOPN: tokenBPriceUSD / this.OPN_PRICE_USD,
          priceInUSD: tokenBPriceUSD,
          priceInverse: reservePriceAinB  // ✅ Use reserve ratio for inverse
        },
        volume24h: volumeOPN,
    ...
    try {
      ...
      for (const baseToken of this.BASE_TOKENS) {
        ...
        if (pairResult.rows.length > 0) {
          ...
          if (basePrice > 0) {
            // ✅ FIXED: Calculate token price based on RESERVES, not swap amounts
            ...
            if (tokenReserve > 0 && baseReserve > 0) {
              // Price = (baseReserve / tokenReserve) * basePrice
  ...
  /**
   * ✅ FIXED: Fallback calculation using RESERVE ratios, not swap amounts
   */
  private static calculateFallbackPrice(
    ...
    // ✅ CRITICAL FIX: Use reserve ratio for price, not swap ratio!
    const reservePriceAinB = reserveB / reserveA;  // Price of A in terms of B
    const reservePriceBinA = reserveA / reserveB;  // Price of B in terms of A
    ...
    if (tokenAIsStable) {
      ...
      tokenAPrice = {
        ...
        priceInUSD: 1,
        priceInverse: reservePriceBinA
      };
      tokenBPrice = {
        priceInOPN: reservePriceBinA / this.OPN_PRICE_USD,
        priceInUSD: reservePriceBinA,
        priceInverse: reservePriceAinB
      };
      ...
      tokenAPrice = {
        priceInOPN: reservePriceAinB / this.OPN_PRICE_USD,
        priceInUSD: reservePriceAinB,
        priceInverse: reservePriceBinA
      };
      volumeUSD = amountAIn * reservePriceAinB;
    } else {
      // Neither is a known stable or WOPN - we can't determine USD price
      // Return with 0 USD values but keep the reserve ratio information
      return {
        pair: this.getPairAddress(tokenA, tokenB),
        token0: tokenA,
        token1: tokenB,
  ...
  private static isStablecoin(tokenAddress: string): boolean {
    ...
    // Check if address contains common stablecoin identifiers
    for (const stable of this.STABLE_COINS) {
      if (addr.includes(stable)) {
        return true;
      }
    }
    return false;
