...
export class TokenAnalyzer {
  ...
  async analyzeToken(tokenAddress: string): Promise<TokenInfo> {
    ...
    try {
      ...
      // ✅ FIXED: Ensure all required fields are present with fallbacks
      const tokenInfo: TokenInfo = {
        address: basicInfo.address || tokenAddress,
        ...
        logoUri: metadata.logoUri || basicInfo.logoUri || undefined,
        website: metadata.website || basicInfo.website || undefined,
        telegram: metadata.telegram || basicInfo.telegram || undefined,
        twitter: metadata.twitter || basicInfo.twitter || undefined,
        description: metadata.description || basicInfo.description || undefined,
        honeypotStatus,
        buyTax: honeypotResult.buyTax,
        sellTax: honeypotResult.sellTax,
        maxBuy: basicInfo.maxBuy,
        maxSell: basicInfo.maxSell,
        isRenounced: basicInfo.isRenounced ?? false,
        isVerified: verificationStatus.isVerified,
        creationBlock: basicInfo.creationBlock,
      ...
      // ✅ FIXED: Return valid TokenInfo with all required fields
      return {
  ...
  private async storeWarnings(tokenAddress: string, warnings: string[]) {
    ...
    try {
      ...
      const tokenExists = await this.db.query(
        'SELECT 1 FROM tokens WHERE LOWER(address) = LOWER($1)',
        [tokenAddress]
      );
      
      if (tokenExists.rows.length === 0) {
        logger.warn(`Cannot store warnings - token ${tokenAddress} not yet in database`);
        return;
      }
      
      for (const warning of warnings) {
        await this.db.query(
          `INSERT INTO token_warnings (token_address, warning_type, message) 
           VALUES ($1, $2, $3) 
           ON CONFLICT (token_address, warning_type) DO UPDATE SET
           message = EXCLUDED.message,
           created_at = CURRENT_TIMESTAMP`,
          [tokenAddress, 'honeypot', warning]
        );
      }
    } catch (error) {
