...
protected async indexToken(tokenAddress: string) {
  ...
  try {
    ...
    if (pairCheck.rows.length > 0) {
      // This is an LP token, treat it specially
    ...
    // For LP tokens, override the symbol to indicate it's an LP
    if (pairCheck.rows.length > 0) {
      tokenInfo.symbol = tokenInfo.symbol || 'LP-TOKEN';
      tokenInfo.name = tokenInfo.name || 'Liquidity Provider Token';
    }
 ...
 protected async handleSwap(log: ethers.Log, args: any, pairInfo: any) {
  ...
  // Fix: Get the maker from the transaction
  const maker = tx?.from || sender || '0x0000000000000000000000000000000000000000';
  ...
  await this.db.query(query, [
    ...
    maker,  // Now properly included
    tradeType,  // Now properly included
    0  // price_impact
  ]);
