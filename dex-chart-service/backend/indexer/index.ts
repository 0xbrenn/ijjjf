...
export class EnhancedDexIndexer {
   ...
   */
    ...
    // Get token info from cache (should already be there)
    const token0Info = this.tokensCache.get(token0.toLowerCase());
    const token1Info = this.tokensCache.get(token1.toLowerCase());
...
protected async indexToken(tokenAddress: string) {
  ...
  try {
    ...
    const finalName = pairCheck.rows.length > 0 ? 'Liquidity Provider Token' : name;

    // STEP 2: INSERT TOKEN FIRST (so it exists in DB)
    const insertQuery = `
      INSERT INTO tokens (
    ...
    logger.info(`✅ Token inserted: ${finalSymbol} (${tokenAddress})`);

    // STEP 3: NOW run full analysis (honeypot scan can store warnings)
    logger.info(`Analyzing token: ${tokenAddress}`);
    const tokenInfo = await this.tokenAnalyzer.analyzeToken(tokenAddress);
    
    // STEP 4: Update token with analysis results
    const updateQuery = `
      UPDATE tokens SET
        logo_uri = $2,
        website = $3,
    ...
    logger.info(`✅ Indexed token: ${finalSymbol} (${tokenAddress})`);
  } catch (error) {
...
protected async handleSwap(log: ethers.Log, args: any, pairInfo: any) {
  ...
  try {
    ...
    const priceImpact = PriceCalculator.calculatePriceImpact(
    ...
    try {
      ...
      for (const token of tokens.rows) {
        ...
        const pairs = await this.db.query(
          'SELECT * FROM pairs WHERE token0 = $1 OR token1 = $1',
          [token.address]
        );
