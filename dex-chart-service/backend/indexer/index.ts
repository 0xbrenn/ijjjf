...
export class EnhancedDexIndexer {
  ...
  protected isSyncing: boolean = true;
...
protected async indexToken(tokenAddress: string) {
   ...
   */
   protected async listenToEvents() {
    ...
    logger.info(`Current block: ${this.currentBlock}`);

    // Get last indexed block from database
    const lastBlockResult = await this.db.query(
      'SELECT MAX(block_number) as last_block FROM trades'
    );
    const lastIndexedBlock = parseInt(lastBlockResult.rows[0]?.last_block || '0');

    logger.info(`Last indexed block: ${lastIndexedBlock}`);

    // If we're behind, catch up first WITHOUT emitting WebSocket
    if (lastIndexedBlock < this.currentBlock - 100) {
      logger.info(`⏳ Catching up ${this.currentBlock - lastIndexedBlock} blocks...`);
      this.isSyncing = true;
      
      // Process historical blocks in batches
      await this.processHistoricalBlocks(lastIndexedBlock + 1, this.currentBlock);
      
      this.isSyncing = false;
      logger.info(`✅ Caught up! Now listening for real-time events.`);
    }

    // Now listen for LIVE events only
    const pairAbi = [
    ...
    this.provider.on('block', async (blockNumber) => {
      ...
      // Process this block in real-time
    });

    const pairs = await this.db.query('SELECT address FROM pairs');
    
    pairs.rows.forEach(({ address: pairAddress }) => {
      const pairInfo = this.pairsCache.get(pairAddress);
      if (!pairInfo) return;

      const contract = new ethers.Contract(pairAddress, pairAbi, this.provider);

      // Listen for live swaps
  ...
  event.log,  // ✅ Use event.log directly - it's already a proper Log object
  ...
  false
    ...
    logger.info('✅ Event listeners attached for real-time');
  }
...
protected async handleSwap(log: ethers.Log, args: any, pairInfo: any, skipWebSocket: boolean = false) {
  ...
  try {
    ...
    // 10. ✅ FIXED: Only emit WebSocket if NOT syncing historical data
    if (!skipWebSocket && this.wsServer) {
      ...
      logger.debug('⏭️  Skipped WebSocket (historical sync)');
    }
...
protected async calculateTokenMetrics() {
  const safeNumber = (value: any, defaultValue: number = 0, maxValue: number = 1e22): number => {
  ...
  if (isNaN(num) || !isFinite(num)) {
    logger.warn(`Invalid number detected: ${value}, using default: ${defaultValue}`);
    return defaultValue;
  }

  // Cap at max value
  if (Math.abs(num) > maxValue) {
    logger.warn(`Value too large: ${num}, capping at ${maxValue}`);
...
// Safe percentage calculator (returns 0-100, never Infinity)
const safePercentage = (value: any, maxValue: number = 100): number => {
  const num = safeNumber(value, 0, maxValue);
  return Math.min(Math.max(num, 0), maxValue);
};

// Safe division (prevents Infinity)
const safeDivide = (numerator: any, denominator: any, defaultValue: number = 0): number => {
  ...
  try {
    ...
    for (const token of tokens.rows) {
      try {  // Add try-catch for individual token
        ...
        for (const pair of pairs.rows) {
          ...
          if (price && price > priceUSD) {
            priceUSD = safeNumber(price, 0);
            priceOPN = safeDivide(price, PriceCalculatorEnhanced.OPN_PRICE_USD, 0);
          }
          ...
          volume24hUSD += safeNumber(volumeResult.rows[0]?.volume, 0);

          const reserve = isToken0 ? pair.reserve0 : pair.reserve1;
          const liquidityTokens = safeNumber(reserve, 0) / 1e18;
        ...
        // Calculate market cap safely
        const tokenInfo = this.tokensCache.get(token.address);
        let marketCap = 0;
        let fdv = 0;

        if (tokenInfo) {
          const circulatingSupply = safeNumber(tokenInfo.circulatingSupply || tokenInfo.totalSupply, 0);
          const totalSupply = safeNumber(tokenInfo.totalSupply, 0);
          
          if (circulatingSupply > 0 && priceUSD > 0) {
            marketCap = safeNumber((circulatingSupply / 1e18) * priceUSD, 0, 1e22);
          }
          
          if (totalSupply > 0 && priceUSD > 0) {
            fdv = safeNumber((totalSupply / 1e18) * priceUSD, 0, 1e22);
          }
        }
        ...
        await this.db.query(query, [
          token.address,
          now,
          safeNumber(priceUSD, 0),
          safeNumber(priceOPN, 0),
          safeNumber(marketCap, 0, 1e22),
          safeNumber(fdv, 0, 1e22),
          safeNumber(volume24hUSD, 0, 1e22),
          safePercentage(priceChanges.change5m, 10000),   // Cap at 10000%
          safePercentage(priceChanges.change1h, 10000),
          safePercentage(priceChanges.change6h, 10000),
          safePercentage(priceChanges.change24h, 10000),
          safePercentage(priceChanges.change7d, 10000),
          safePercentage(priceChanges.change30d, 10000),
          safeNumber(totalLiquidityUSD, 0, 1e22),
          safeNumber(totalLiquidityOPN, 0, 1e22),
          safeNumber(holderMetrics.holderCount, 0),
          safeNumber(txMetrics.txCount, 0),
          safeNumber(txMetrics.uniqueBuyers, 0),
          safeNumber(txMetrics.uniqueSellers, 0),
          safePercentage(txMetrics.buyPressure, 100)      // Cap at 100%
        ]);
        ...
        await this.redis.setex(
          ...
          300,
          JSON.stringify({
            priceUSD: safeNumber(priceUSD, 0),
            priceOPN: safeNumber(priceOPN, 0),
            marketCap: safeNumber(marketCap, 0),
            fdv: safeNumber(fdv, 0),
            volume24hUSD: safeNumber(volume24hUSD, 0),
            priceChange24h: safePercentage(priceChanges.change24h, 10000),
            liquidity: safeNumber(totalLiquidityUSD, 0),
            holders: safeNumber(holderMetrics.holderCount, 0)
          })
        );
      } catch (tokenError) {
        logger.error(`Error processing token ${token.address}:`, tokenError);
        // Continue to next token instead of crashing
        continue;
      }
    }
  ...
  for (const { name, seconds } of timeframes) {
    ...
    if (previousPrice && previousPrice > 0 && currentPrice) {
      ...
      changes[name] = Math.max(Math.min(change, 10000), -10000);
    } else {
      changes[name] = 0;
    }
  }
  ...
  // Safe buy pressure calculation (0-100%)
  let buyPressure = 0;
  if (totalVolume > 0) {
    buyPressure = Math.min((buyVolume / totalVolume) * 100, 100);
  }
  ...
  return {
    ...
    buyPressure: isFinite(buyPressure) ? buyPressure : 0
  };
}
  ...
  protected async startLiquidityTracking() {
    ...
    protected async processHistoricalBlocks(fromBlock: number, toBlock: number) {
    ...
    for (let start = fromBlock; start <= toBlock; start += BATCH_SIZE) {
      ...
      const swapFilter = {
        address: Array.from(this.pairsCache.keys()),
      ...
      // Process them WITHOUT emitting WebSocket
      for (const log of logs) {
        ...
        if (parsed) {
          // Process swap but skip WebSocket emission during sync
          await this.handleSwap(log, parsed.args, pairInfo, true); // ← Pass skipWebSocket flag
        }
      }
