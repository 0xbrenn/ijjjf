...
app.get('/api/pairs/:address', async (req: Request, res: Response) => {
  try {
    ...
    // Get pair info
  // Around line 250 in backend/api/server.ts
const pairResult = await db.query(
  ...
  [address]
    ...
    // ✅ FIXED: Determine which token is WOPN to show correct prices
  ...
  isVerified: pair.token0_contract_verified || false
      ...
      // ✅ FIXED: Show correct token's price based on which one is WOPN
      recentTrades: tradesResult.rows.map(trade => {
        // Always show the NON-WOPN token's price
        const displayPriceUSD = token0IsWOPN 
          ? parseFloat(trade.price_token1_usd || '0')  // Token1 is the other token
          : parseFloat(trade.price_token0_usd || '0'); // Token0 is the other token
        ...
        return {
          ...
          priceUSD: displayPriceUSD,  // ✅ Now shows correct token's price
          priceOPN: displayPriceOPN,  // ✅ Now shows correct token's price
