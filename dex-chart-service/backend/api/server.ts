  ...
  try {
    ...
    if (token1IsWOPN) {
      ...
      if (token0Price > 0 && token1Price > 0) {
        ...
        // Only token0 has price
        baseTokenIndex = 0;
      } else if (token1Price > 0) {
        // Only token1 has price
        baseTokenIndex = 1;
      }
      // else: neither has price, default to token0
    }
    ...
    const response = {
      ...
      token0: {
        ...
        isRenounced: false, // Not tracked in DB
      ...
      // ✅ NEW: Add base token metadata for frontend reference
      baseToken: baseTokenIndex === 0 ? 'token0' : 'token1',
      baseTokenSymbol: baseTokenIndex === 0 ? pair.token0_symbol : pair.token1_symbol,
      quoteTokenSymbol: baseTokenIndex === 0 ? pair.token1_symbol : pair.token0_symbol,
      liquidity: {
      ...
      // ✅ FIXED: Consistently show base token's price across all trades
      recentTrades: tradesResult.rows.map(trade => {
        // Always show the base token's price (the one being priced in the pair)
        const displayPriceUSD = baseTokenIndex === 0
          ? parseFloat(trade.price_token0_usd || '0')  // Show token0 (e.g., OPNT)
          : parseFloat(trade.price_token1_usd || '0'); // Show token1 (e.g., tBNB when WOPN is token0)
        ...
        return {
          ...
          priceUSD: displayPriceUSD,  // ✅ Now consistent - always shows base token price
          priceOPN: displayPriceOPN,  // ✅ Now consistent - always shows base token price
