...
app.get('/api/search', async (req: Request, res: Response) => {
  try {
    ...
    // Find matching tokens (exclude WOPN from results)
    const tokensResult = await db.query(
      `SELECT 
       ...
       WHERE (LOWER(t.symbol) LIKE LOWER($1) 
          ...
          OR LOWER(t.address) = LOWER($2))
         AND LOWER(t.address) != LOWER($3)
       ORDER BY tm.volume_24h_usd DESC NULLS LAST
       LIMIT $4`,
      [searchTerm, q, WOPN_ADDRESS, limit]
    );

    // For each matching token, get all its pairs
    const results = await Promise.all(
      tokensResult.rows.map(async (token) => {
        const pairsResult = await db.query(
          `SELECT 
            ...
            tm1.price_opn as token1_price_opn,
            tm0.liquidity_usd as liquidity_usd,
            COALESCE(tm0.volume_24h_usd, 0) + COALESCE(tm1.volume_24h_usd, 0) as pair_volume_24h
           FROM pairs p
           JOIN tokens t0 ON LOWER(p.token0) = LOWER(t0.address)
           JOIN tokens t1 ON LOWER(p.token1) = LOWER(t1.address)
           LEFT JOIN LATERAL (
           ...
           WHERE LOWER(p.token0) = LOWER($1) OR LOWER(p.token1) = LOWER($1)
           ORDER BY pair_volume_24h DESC
           LIMIT 10`,
          [token.address]
        );

        const pairs = pairsResult.rows.map(pair => {
          // Determine which token is the base (not the searched token) or WOPN
          const isToken0Match = pair.token0.toLowerCase() === token.address.toLowerCase();
          const token0IsWOPN = pair.token0.toLowerCase() === WOPN_ADDRESS.toLowerCase();
          const token1IsWOPN = pair.token1.toLowerCase() === WOPN_ADDRESS.toLowerCase();

          let pairedWithSymbol, pairedWithName, pairDisplay;
          
          if (isToken0Match) {
            // Searched token is token0, show token1
            pairedWithSymbol = pair.token1_symbol;
            pairedWithName = pair.token1_name;
            pairDisplay = `${token.symbol}/${pair.token1_symbol}`;
          } else {
            // Searched token is token1, show token0
            pairedWithSymbol = pair.token0_symbol;
            pairedWithName = pair.token0_name;
            pairDisplay = `${token.symbol}/${pair.token0_symbol}`;
          }
          ...
          return {
            ...
            volume24h: parseFloat(pair.pair_volume_24h || '0')
          };
        });
        ...
        return {
          ...
          pairs: pairs // All pairs for this token
        };
      })
    );
...
app.get('/api/market/summary', async (req: Request, res: Response) => {
  try {
    // Get total pairs count
    const pairsCount = await db.query('SELECT COUNT(*) as total FROM pairs');
    
    // Get 24h volume
    const volumeResult = await db.query(`
      SELECT SUM(amount_in_usd) as total_volume
      FROM trades
      WHERE timestamp > EXTRACT(EPOCH FROM NOW() - INTERVAL '24 hours')
    `);
    
    // Get active traders (unique addresses in last 24h)
    const tradersResult = await db.query(`
      SELECT COUNT(DISTINCT maker) as active_traders
      FROM trades
      WHERE timestamp > EXTRACT(EPOCH FROM NOW() - INTERVAL '24 hours')
    `);

    res.json({
      totalPairs: parseInt(pairsCount.rows[0]?.total || '0'),
      volume24h: parseFloat(volumeResult.rows[0]?.total_volume || '0'),
...
/**
 * Most Active Pairs (by trade count in timeframe)
 */
app.get('/api/pairs/active', async (req: Request, res: Response) => {
  try {
    ...
    const hours = timeframeMap[timeframe] || 1;
    ...
    const result = await db.query(
      `SELECT 
        ...
        COUNT(tr.id) as trades_count
       FROM pairs p
       JOIN tokens t0 ON p.token0 = t0.address
       JOIN tokens t1 ON p.token1 = t1.address
       JOIN trades tr ON tr.pair_address = p.address
       WHERE tr.timestamp > EXTRACT(EPOCH FROM NOW() - INTERVAL '${hours} hours')
       GROUP BY p.address, t0.symbol, t0.name, t0.address, t1.symbol, t1.name, t1.address
    ...
    const pairs = result.rows.map(row => ({
      ...
      [`trades${timeframe}`]: parseInt(row.trades_count)
    }));

    res.json({ pairs });
  } catch (error) {
    logger.error('Error fetching active pairs:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

/**
 * Enhanced Pair Details with additional analytics
 */
app.get('/api/pairs/:address/details', async (req: Request, res: Response) => {
  try {
    const { address } = req.params;
    ...
    const statsResult = await db.query(
      `SELECT 
        ...
        MAX(price_token0_usd) as high_24h
       FROM trades 
       WHERE LOWER(pair_address) = LOWER($1)
       AND timestamp > EXTRACT(EPOCH FROM NOW() - INTERVAL '24 hours')`,
      [address]
    );
    ...
    const holdersResult = await db.query(
      `SELECT holder_address as address, percentage
       FROM token_holders
    ...
    res.json({
      ...
      volume24h: parseFloat(stats.volume_usd || '0'),
      liquidity: parseFloat(pair.reserve0 || '0') * parseFloat(pair.token0_price_usd || '0') * 2,
      trades24h: parseInt(stats.trade_count || '0'),
      uniqueTraders24h: parseInt(stats.unique_traders || '0'),
      high24h: parseFloat(stats.high_24h || '0'),
      low24h: parseFloat(stats.low_24h || '0'),
      topHolders: holdersResult.rows.map(h => ({
        address: h.address,
        percentage: parseFloat(h.percentage || '0')
      })),
      liquidityLocked: false, // TODO: Implement liquidity lock detection
      contractVerified: false // TODO: Implement contract verification check
    });
  } catch (error) {
    logger.error('Error fetching pair details:', error);
    res.status(500).json({ error: 'Internal server error' });
