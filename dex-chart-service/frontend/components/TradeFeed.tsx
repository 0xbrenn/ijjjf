...
interface TradeFeedProps {
  ...
  baseTokenSymbol?: string  // Pass this from parent component
}
...
export default function TradeFeed({ pairAddress, priceDisplay, baseTokenSymbol = 'TOKEN' }: TradeFeedProps) {
  ...
  useEffect(() => {
    if (initialTrades) {
      setTrades(initialTrades.slice(0, 100))
    }
  }, [initialTrades])
  ...
  // ✅ FIXED: Calculate actual token amount from USD volume and price
  const calculateTokenAmount = (trade: Trade): number => {
        ...
        {trades.length === 0 ? (
          ...
          trades.map((trade, index) => (
            ...
            >
              ...
              {/* Price - Always shows token price in selected currency */}
              <div className="w-28 text-right">
                ...
                <div className="font-mono text-sm">
                  {priceDisplay === 'usd' ? (
                    ...
                    `${trade.priceOPN.toFixed(4)} OPN`
                  )}
                </div>
              </div>

              {/* Amount - Shows actual token amount traded */}
              <div className="w-28 text-right">
                <div className="text-xs text-gray-400">Amount</div>
                <div className="font-mono text-sm">
                  {/* ✅ FIXED: Calculate token amount correctly */}
                  {calculateTokenAmount(trade).toFixed(2)} {baseTokenSymbol}
                </div>
              </div>
              ...
              <div className="w-32 text-right">
                ...
                <a
                  href={`https://testnet.iopn.tech/address/${trade.maker}`}
                  target="_blank"
              ...
              <div className="w-8">
                <a
                  href={`https://testnet.iopn.tech/tx/${trade.hash}`}
                  target="_blank"
                  rel="noopener noreferrer"
